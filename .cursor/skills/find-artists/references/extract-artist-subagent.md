...
Skill directory: /Users/samreay/repos/Samreay/.cursor/skills/find-artists
...
5. If attribution is clear, pick a cover stem using existing
   ...
   `src/assets/img/covers/` AND `.cursor/skills/find-artists/tmp_covers/` to
   avoid collisions.
6. Download the best front-cover image from the markdown's `## Image URLs`
   section into
   `.cursor/skills/find-artists/tmp_covers/<stem>.(jpg|jpeg|png|webp)`.
...
`uv run .cursor/skills/find-artists/scripts/sync_covers.py` after your edit
