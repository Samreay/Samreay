...
def audit() -> dict[str, list[str]]:
    ...
    duplicate_stems_in_ts = sorted({s for s in stems_lower if stems_lower.count(s) > 1})
...
def apply_moves() -> int:
    ...
    for path in sorted(TMP_COVERS.iterdir()):
        ...
        if dest.exists():
            print(f"[sync_covers] refusing to overwrite existing {dest.relative_to(REPO_ROOT)}")
            continue
