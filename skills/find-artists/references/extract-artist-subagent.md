...
Skill directory: /Users/samreay/repos/Samreay/skills/find-artists

Goal:
Read exactly one queued markdown file,
`skills/find-artists/references/to_extract/<id>.md`, decide whether it
contains a clear cover artist attribution, and if possible update the site
data. Do not inspect other queued markdown files.

Workflow:
1. Read `skills/find-artists/references/to_extract/<id>.md`.
...
4. If attribution is missing or ambiguous, move the markdown unchanged to
   `skills/find-artists/references/extracted/<id>.md`, make no
...
5. If attribution is clear, pick a cover stem using existing
   ...
   `src/assets/img/covers/` AND `skills/find-artists/tmp_covers/` to
   avoid collisions.
...
8. Move the markdown to
   `skills/find-artists/references/extracted/<id>.md`.
...
`uv run skills/find-artists/scripts/sync_covers.py` after your edit
