...
All scripts live in `skills/parse-flowchart/scripts/` and are
self-contained `uv run` scripts (inline PEP 723 metadata). Outputs land
under `skills/parse-flowchart/work/<image-stem>/` (gitignored).
...
```bash
uv run skills/parse-flowchart/scripts/prepare.py path/to/Story-Finder-big.jpg
```
...
```bash
uv run skills/parse-flowchart/scripts/crop.py path/to/Story-Finder-big.jpg \
  --center 18000,12000 --size 4000,3000 --pad 200 \
  --out skills/parse-flowchart/work/Story-Finder-big/zoom_e_start_dotf.png
```
...
```bash
uv run skills/parse-flowchart/scripts/verify.py \
  --state    skills/parse-flowchart/work/Story-Finder-big/state.json \
  --prepared skills/parse-flowchart/work/Story-Finder-big/
```
...
```bash
uv run skills/parse-flowchart/scripts/emit.py \
  --state skills/parse-flowchart/work/Story-Finder-big/state.json \
  --out  skills/parse-flowchart/work/Story-Finder-big/flowchart.generated.ts
```
