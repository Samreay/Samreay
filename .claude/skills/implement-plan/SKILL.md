...
4. **Verify.** Run:
   ...
   make verify-phase-N
   ```
   This is a wrapper around `uv run .claude/skills/implement-plan/scripts/verify.py --phase N`. It writes a structured report to `state/phase-N-report.json` and prints a human summary.
...
7. **Pause.** Ask the user whether to advance to phase N+1, retry phase N (e.g. for additional polish), or stop the session. Do not auto-advance.
...
3. For visual diffs, open `.claude/skills/implement-plan/scripts/visual/test-results/` and inspect the `actual.png`, `expected.png`, and `diff.png` triplet.
